...
def method_active(method, sources):
    return (method.get('availability', 'active') == 'active'
            and all(sources[sid].get('availability', 'active') == 'active' for sid in method['source_ids']))


def active(entry, sources, methods):
    return (entry.get('availability', 'active') == 'active'
            and sources[entry['source_id']].get('availability', 'active') == 'active'
...
def search(index, library, genre_name, query='', stage='draft', limit=3):
    ...
    sources = {s['source_id']: s for s in library['sources']}
    ...
    for score, entry, method, source in ranked:
        ...
        results.append({'person_id': entry['person_id'], 'name': source['name'], 'genre_fit': entry['fit'], 'work_evidence': entry['work_evidence'], 'source': {k: source.get(k) for k in ('url', 'locator', 'evidence_summary', 'read_status', 'accessed_at')}, 'method': selected_method})
...
def main():
    ...
    if not (args.genre or args.list_genres):
    ...
    try:
        ...
        result = validate_index(index, library) if args.list_genres else search(index, library, args.genre, args.query, args.stage, args.limit)
