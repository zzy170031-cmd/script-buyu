...
此前采用的screenwriting-skills已有18模块本地写作适配，YouMind图像提示词也已保存可离线查询的全文快照；来源、版本、许可和使用入口一并列入上述说明。无需另外安装这些来源Skill；外部URL用于溯源，公开原书及上游示例图片不冒充已完整内置。

已将Skill目录单独复制测试：94项回归、资料完整性、六组实际查询和官方Skill结构校验均通过，85条当前参考链接均在Skill内部。详见 [独立安装与内置资料验证](docs/独立安装与内置资料验证.json)；该记录验证打包及工具可用性，不代替真实剧本与人物图片的内容验收。
